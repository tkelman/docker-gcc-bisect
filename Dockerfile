...
RUN zypper -n install mingw32-libstdc++6
...
RUN /opt/llvm/llvm-3.7.1.src/configure --host=i686-w64-mingw32 \
    --enable-optimized --enable-targets=host CXXFLAGS=-fno-ipa-cp
#CXXFLAGS=-fno-devirtualize
#> llvm-conf.log 2>&1 || exit 125
...
RUN make -j`nproc` ONLY_TOOLS=opt
#> llvm-build.log || exit 125
